      ...
      <div className='nodeBox'>
        ...
        {nodeState ? <>
          <div className='equityBox'>
            ...
            <div> <img src={equityIcon} alt="" /> {t('10000U矿机放大3倍3万U额度,0.8%每日释放删除掉')} </div>
            <div> <img src={equityIcon} alt="" /> {t('全网动静产币均分红2%')}</div>
            <div> <img src={equityIcon} alt="" /> {t('全网众筹静态10%产出总额x 1%')} </div>
            <div> <img src={equityIcon} alt="" /> {t('获得F6会员等级(限时3个月)')} </div>
          </div>
        </> : <>
          <Button className='payNode' onClick={() => {}}>{t('选购节点')}</Button>
        </>}
      </div>
    </>
  )
